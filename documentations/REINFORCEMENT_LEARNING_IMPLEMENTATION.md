...
The action space consists of 21 crops commonly grown in North Karnataka:
...
5. **_get_state_key(features)**: Converts continuous features to discrete state representation
6. **_initialize_state(state_key)**: Initializes Q-values for new states with small random values
...
Training script location: `scripts/train_reinforcement_model.py`
...
2. Selectable via the `model_name` parameter ("Reinforcement Learning")
...
Located at `frontend/src/pages/RLFeedback.jsx`, this page provides a user-friendly interface for farmers to submit feedback:

Key features:
- Form for entering soil and weather conditions
- Fields for recommended vs actual crops
- Success indicator checkbox
- Real-time feedback submission to backend
- Success/error messaging

### How It Works Section

The page includes educational content explaining the reward system:
- +1.0 reward for perfect matches
- +0.5 reward for partial successes
- -1.0 reward for failures

## Technical Implementation Details

### State Discretization

The `_get_state_key` method converts continuous environmental features into discrete bins:
...
The model calculates confidence scores based on the range of Q-values:
- For single predictions: Normalized difference between max and min Q-values
- For top recommendations: Relative ranking with normalized Q-values

### Model Persistence

The model is automatically saved after each feedback update:
- Main model: `app/models/rl_model.joblib`
- Soil encoder: `app/models/rl_soil_encoder.joblib`
- Metadata: `app/models/rl_model_metadata.json`
...
4. Monitor model statistics through the model's `get_stats()` method
...
├── app/
│   ├── main.py                 # RL model class and endpoints
│   └── models/
│       ├── rl_model.joblib     # Serialized RL model
│       ├── rl_soil_encoder.joblib # Soil type encoder
│       └── rl_model_metadata.json # Model metadata
├── scripts/
│   └── train_reinforcement_model.py # Training script
└── frontend/
    └── src/
        └── pages/
            └── RLFeedback.jsx   # Feedback interface
```
